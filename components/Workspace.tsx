...
const Workspace: React.FC = () => {
  ...
  useEffect(() => {
    // This handles the initial load and checks if user is logged in
  ...
  const handleGeneration = async () => {
    ...
    const BACKEND_URL = "https://wisely-hazelly-cherly.ngrok-free.dev/generate-mesh/";
  ...
  // --- EXPORT AND DRAWING FUNCTIONS (unchanged) ---
  ...
  // 1. Show a loading screen while Firebase checks auth
  if (authLoading) {
    return (
  ...
  // 2. If NOT logged in (currentUser is null), show Auth screens
  // This is what users see if they navigate to /workspace manually
  if (!currentUser) {
    if (authScreen === 'login') {
  ...
  // 3. If we are here, user IS logged in. Show the main app.
  return (
    <div className="flex flex-col h-screen font-sans bg-base-100 text-content antialiased">
      {/* Pass the user and the NEW sign out function to the Header */}
